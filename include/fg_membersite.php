...
class FGMembersite
{
	...
	function Survey()
    {
        ...
        $this->CollectSurveySubmission($formvars);
		
		if(!$this->SaveSurveyToDatabase($formvars))
        {
            return false;
        }
	...
	function MailingListUser()
    {
        ...
        $this->CollectMailingListSubmission($formvars);
        
        if(!$this->InsertMailingListIntoDB($formvars))
        {
            return false;
        }
        
        $this->SendMailingListEmail($formvars);
	...
	function CollectMailingListSubmission(&$formvars)
    {
		...
		$formvars['lastname'] = $this->Sanitize($_GET['l']);
        $formvars['email'] = $this->Sanitize($_GET['e']);        
    }
	...
	function SendMailingListEmail(&$formvars)
    {
	...
	function CollectSurveySubmission(&$formvars)
    {
        $formvars['sid'] = $this->Sanitize($_POST['name']);
    }
	...
	function SaveSurveyToDatabase(&$formvars)
    {
        if(!$this->DBLogin())
        {
            $this->HandleError("Database login failed!");
        ...
        if(!$this->UpdateSurveyInDB($formvars))
        {
            $this->HandleError("Update Survey in Database failed!");
            return false;
        }
        return true;
    }
	...
	function UpdateSurveyInDB(&$formvars)
    {
        $update_query = 'UPDATE survey SET sid="' . $this->SanitizeForSQL($formvars['sid']) . '"';
        if(!mysql_query( $update_query ,$this->connection))
        {
            $this->HandleDBError("Error updtingng the survey data to the table\nquery:$update_query");
            return false;
        }        
        return true;
    }
 ...
 /*
    ...
    */
	...
	function GetAuthorsWHoleName($id)
	{
		...
		while ($row_itemauthors = mysql_fetch_assoc($itemauthors_results)) {
			if ($row_itemauthors['title'] != "")
				$strAuthors = $strAuthors." ".$row_itemauthors['title'];
			if ($row_itemauthors['fname'] != "")
				$strAuthors = $strAuthors." ".$row_itemauthors['fname'];
			if ($row_itemauthors['mname'] != "")
				$strAuthors = $strAuthors." ".$row_itemauthors['mname'];
			if ($row_itemauthors['lname'] != "")
				$strAuthors = $strAuthors." ".$row_itemauthors['lname'];
			if ($row_itemauthors['suffix'] != "")
				$strAuthors = $strAuthors." ".$row_itemauthors['suffix'];
